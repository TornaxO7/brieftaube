...
const PANEL_TITLE: &str = "Mails";
const PREVIEW_PANEL_TITLE: &str = "Mail content";

#[derive(Default)]
pub struct ThreadMails {}

impl StatefulWidget for ThreadMails {
...
impl ThreadMails {
    fn render_mail_list(&self, area: Rect, buf: &mut Buffer, state: &mut super::State) {
        match state.mails.as_ref() {
            ...
            None => Widget::render(
                Paragraph::new("Loading...").block(Block::bordered().title(PANEL_TITLE)),
                area,
                buf,
            ),
        }
    ...
    fn render_preview(&self, area: Rect, buf: &mut Buffer, state: &mut super::State) {
        if let (Some(root_mails), Some(idx)) = (&state.mails, state.list_state.selected) {
